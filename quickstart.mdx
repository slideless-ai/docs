...
Upload the folder — the CLI walks it, hashes every file, and uploads only what's missing on the backend (later updates will skip unchanged files):

```bash
slideless share ./my-deck --title "My first deck"
```
...
Edit your deck (change the HTML, swap an image), then re-publish to the same URL:

```bash
# Same path as your original share
slideless update 0192f1c3-... ./my-deck
```

Recipients see the new content on next load. The URL doesn't change, the view count is preserved, and only files whose contents actually changed re-upload — the rest are deduplicated. A one-image swap on a 200 MB deck transfers ~the size of that one image, not 200 MB.
