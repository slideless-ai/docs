---
title: "The Slideless MCP server"
description: "Connect any MCP host — Claude Desktop, claude.ai, ChatGPT desktop, Cursor — to Slideless. Sign in with your Slideless account to create, upload, share, and manage presentations directly from your chats — no CLI required."
---

Slideless ships a remote MCP server at `https://mcp.slideless.ai/mcp`. Add it to any host that supports MCP custom connectors — Claude Desktop, claude.ai, ChatGPT desktop, Cursor — and talk to Slideless from inside your chats. No CLI install, no API key to copy. You add the connector once, sign in to Slideless when prompted, and ask things like *"upload this HTML as a deck"* or *"share my latest presentation with bob@acme.com"*.

<Note>
Using Slideless from the **terminal**? See the [`slideless` CLI](/cli/overview).
Using it from **Claude Code or Cursor**? The [plugin skills](/skills/overview) are the richer path, though the MCP server works there too.
</Note>

## Add the connector

The walkthrough below uses Claude Desktop; the same URL works in any MCP host (see [Other MCP hosts](#other-mcp-hosts)).
...
To cut off Claude's access, remove the connector from Claude Desktop: open the connector list, find the Slideless entry, and delete it. Claude can no longer call the Slideless tools once the connector is gone.

To reconnect later, add the connector again and complete the sign-in handshake.

## Troubleshooting

<Note>
If the connector setup fails with **"Couldn't reach the MCP server"**, double-check the URL is exactly `https://mcp.slideless.ai/mcp` (with `/mcp` at the end). Then retry. The OAuth handshake can briefly fail on first connect; remove the connector and re-add it once.
</Note>
