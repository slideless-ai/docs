...
You already have a `shareId` (from [`POST /commitPresentationVersion`](/api-reference/commit-presentation-version) or `slideless share`) and you want to email it to one or more recipients. The endpoint:
...
`data.failed[]` contains **per-recipient** failures inside an otherwise-200 response. The endpoint only returns a non-2xx status when a preflight condition fails (auth, not-found, archived, too-many-recipients, …) — i.e. when no email could be attempted for any recipient.
...
- [`POST /commitPresentationVersion`](/api-reference/commit-presentation-version) — to create the `shareId` in the first place
