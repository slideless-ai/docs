...
<Note>
For most use cases, the [`slideless` CLI](/cli/overview) is easier. The CLI command is `slideless pin <shareId> <tokenId> --to-version <N>` or `--latest`.
</Note>
...
|---|---|---|---|
| `shareId` | string | yes | Presentation the token belongs to |
...
| `404` | `not-found` | `shareId` or `tokenId` doesn't exist |
