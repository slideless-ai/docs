...
| `410` | Gone — share expired, presentation archived, or token revoked |
...
| [`/listPresentationVersions/{shareId}`](/api-reference/list-presentation-versions) | GET | API key (owner) | List every version's summary |
| [`/getPresentationVersion/{shareId}/{version}`](/api-reference/get-presentation-version) | GET | API key (owner) | Get a specific version's full manifest |
| [`/revokeSharedPresentation`](/api-reference/revoke-presentation) | POST | API key | Revoke one token or archive the presentation |
...
| Endpoint | Method | Auth | Purpose |
|---|---|---|---|
| [`/getSharedPresentation/{shareId}/_t/{token}/...`](/api-reference/get-shared-presentation) | GET | Token in URL | Serves entry HTML + assets. Range-request aware for video. |
