---
title: "GET /getPresentationVersion/{shareId}/{version}"
...
```
GET https://europe-west1-slideless-ai.cloudfunctions.net/getPresentationVersion/{shareId}/{version}
```
...
| Status | Code | Cause |
|---|---|---|
| `400` | `invalid-argument` | Path is not `/{shareId}/{version}` or `version` isn't a positive integer |
| `401` | `unauthenticated` | Missing/invalid API key |
| `403` | `permission-denied` | Not the presentation's owner |
| `404` | `not-found` | `shareId` doesn't exist, `version` exceeds `currentVersion`, or manifest file missing |
