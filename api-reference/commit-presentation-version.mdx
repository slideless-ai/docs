...
<Note>
For most use cases, the [`slideless` CLI](/cli/overview) handles the upload flow end-to-end. The CLI command is `slideless share <path>` or `slideless update <shareId> <path>`.
</Note>
...
|---|---|---|---|
| `shareId` | string | one of | Existing presentation — bumps `currentVersion` and writes a new manifest. |
| `sessionId` | string | one of | Session from `precheckAssets` — creates a new presentation on this session's reserved shareId. |
...
| `403` | `permission-denied` | Key lacks `presentations:write`, or shareId owned by another user |
