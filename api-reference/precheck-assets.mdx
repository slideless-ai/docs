...
| `shareId` | string | no | Check against this existing presentation's blob store. Requires ownership. |
| `sessionId` | string | no | Carry a session from a prior call. Mutually exclusive with `shareId`. |

Call patterns:

- **New presentation (first call):** omit both `shareId` and `sessionId`. Response includes `sessionId` + reserved `shareId` to use for `uploadPresentationAsset` and `commitPresentationVersion`.
- **Update existing presentation:** pass `shareId`. Backend checks dedup against that presentation's blob store.
...
### New-presentation flow (no shareId in request)
...
### Update flow (shareId in request)
...
| `shareId` | Only present on new-presentation flow. The reserved shareId for the final commit. |
...
| `403` | `permission-denied` | Key lacks `presentations:write`, or shareId/sessionId owned by another user |
| `404` | `not-found` | `shareId` or `sessionId` doesn't exist (sessions expire after 1 hour) |
| `410` | `archived` | Presentation is archived — cannot add assets |
