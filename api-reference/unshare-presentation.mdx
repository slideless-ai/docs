...
`Authorization: Bearer <key>` — owner, or an active dev collaborator. A dev may revoke only a single token they created (pass `tokenId`); revoking all tokens at once is owner-only.
...
| 403 | `permission-denied` | Not the owner or an active collaborator (or a dev trying to revoke all / another's link) |
