...
| Header | Value |
|---|---|
| `Authorization` | `Bearer cko_…` (or `cka_…`) — must belong to the owner or an active dev collaborator |
...
| `canAnnotate` | boolean | no | Default `false`. When `true`, mints an **annotator link** — the holder gets the annotation overlay in the viewer and can leave notes. Requires a **non-empty** `tokenName` (it becomes the note author). See [Share tokens › Annotator links](/concepts/share-tokens#annotator-links). |
...
Example — annotator link (reviewer can leave notes):
...
|---|---|---|---|
| `400` | `invalid-argument` | `presentationId` or `tokenName` missing or wrong type / empty, or `canAnnotate: true` sent with an empty `tokenName` | Send both as non-empty strings; supply a `tokenName` whenever `canAnnotate` is true |
| `401` | `unauthenticated` | Missing or invalid API key | Set a valid `Authorization` header |
| `403` | `permission-denied` | Key's user is not the owner or an active collaborator | Use a key from the owner's or a collaborator's account |
