...
| **`share-presentation`** | Mint a public viewer URL for an existing deck. Trigger phrases: "share a link", "get a public URL". Owner or active dev collaborator. | `slideless share <presentationId>` |
| **`unshare-presentation`** | Revoke viewer URLs — every token, or one with `--token`. The deck stays editable. Owner (any/all); dev (only links they created). | `slideless unshare <presentationId>` |
...
<Note>
For the `[ ... edit, regenerate, iterate ... ]` step, run [`slideless dev <folder>`](/cli/commands#local-development) in your terminal: it serves the deck locally and auto-reloads the browser on every save — no `push` between edits. It's a long-running local server, so you run it yourself rather than via the agent. Publish with `push-presentation` when you're done.
</Note>
...
- **[The Slideless MCP server](/integrations/mcp-server)** — The non-CLI alternative for any MCP host (Claude Desktop, claude.ai, ChatGPT desktop, Cursor).
