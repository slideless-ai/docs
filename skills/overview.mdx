...
| **`share-presentation`** | Upload a deck (folder OR single HTML file) to Slideless and return a public share URL. Optionally update an existing share in place. | `slideless share <path>` (or `slideless update <shareId> <path>`) |
...
| **`update-presentation`** | Replace the deck behind an existing share URL (folder or file). Same URL, view counts preserved, version bumped, unchanged assets deduplicated. | `slideless update <shareId> <path>` |
...
> /skill share-presentation
  ...
  → title: "Q4 Results"
  → ran: slideless share ./q4-results --title "Q4 Results" --json
...
> /skill update-presentation
  ...
  → ran: slideless update 0192f1c3-... ./q4-results --json
  → updated to v2 (same URL, unchanged assets deduplicated)
