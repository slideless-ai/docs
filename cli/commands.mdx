...
- Notes are stored in **`.slideless/annotations.json`** at the deck root. That folder is created on first save and is **never uploaded** by `push` (it's in the built-in ignore list), and writing to it does **not** trigger a live-reload.
- A 📝 badge (bottom-right) shows the count of unprocessed notes. Click it to open a side panel where you can **edit**, **remove**, **jump to** a note's location, and switch between **New** and **Previous** (a `processed` flag on each note).
...
```json
{
  ...
  "selectedText": "Our architecture scales",
