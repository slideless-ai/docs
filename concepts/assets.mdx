...
```
shared_presentations/{shareId}/assets/sha256/{first-two-hex-chars}/{full-hash}
```
...
When the viewer fetches `/share/{shareId}/images/hero.jpg`, the backend resolves the current (or pinned) version's manifest, looks up the literal path, reads the hash, and streams the blob. No HTML rewriting, no URL mangling — the author's `<img src="./images/hero.jpg">` goes straight through.

## Relative paths and the deck root

The deck root is whatever you passed to `slideless share`. Paths in HTML and CSS are resolved relative to the file they appear in:
...
- Deduped assets on `slideless update` — if an image is unchanged, it doesn't re-upload and doesn't re-charge storage.
