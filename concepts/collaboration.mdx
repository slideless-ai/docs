...
| **Dev collaboration** | Edit access plus sharing management — an authenticated collaborator can push versions and manage the deck's share links | `slideless invite <id> --email <addr>` |
...
- **owner** — the original creator. Can do everything, including invite/uninvite collaborators, delete the deck, and publish to the marketplace.
- **dev** — invited collaborator. Can pull, push new versions, mint viewer + annotator links, pin tokens, share by email, and pull annotations. Can revoke only links they created. **Cannot** invite/remove other collaborators, revoke all links at once, delete the deck, or publish to the marketplace.
