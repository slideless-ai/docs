...
```bash
slideless share ./deck.html --title "My talk"
```
...
```bash
slideless share ./deck --title "My talk"
```

The CLI walks the folder and uploads every file that isn't in the default ignore list (`.git/`, `node_modules/`, `.DS_Store`, `*.log`, etc.) plus whatever a `.slidelessignore` file at the folder root excludes. Parent-directory references (`<img src="../outside/foo.jpg">`) are rejected at upload time — decks must be self-contained within their root.
...
Under the hood, each file is stored at a path derived from its SHA-256 hash. When you update a deck, only the files whose contents actually changed re-upload — everything else dedupes. A five-version deck with one changed image across versions costs you ~1.1× the size, not 5×.
...
1. **Upload** via `slideless share <path>` — creates a new presentation, mints a default token whose `versionMode` is `latest`.
2. **View**: anyone with the token URL renders the deck in a sandboxed iframe; view count increments server-side.
3. **Update** via `slideless update <shareId> <path>` — creates a new immutable version, bumps `currentVersion`, preserves tokens + view counters, dedupes unchanged files.
4. **Pin** via `slideless pin <shareId> <tokenId> --to-version <N>` — freeze a specific recipient on a specific version so later updates don't affect their view.
5. **Revoke** from the dashboard — individual tokens return 403, or archive the whole presentation (all tokens dead, cannot un-archive).
...
- In-dashboard upload — v3 uploads are CLI-only. The dashboard handles browsing, previewing, token management, and archiving.
