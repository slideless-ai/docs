...
2. **Share** it (folder or single file) and get back a public URL to send to anyone.
3. **Update** an existing share in place. Same URL, new content, unchanged assets deduped.
4. **List** and **get** your shared presentations (with per-token view counts and version info).
...
Claude writes either a single self-contained HTML file or a deck folder (if the content needs images, video, or 3D assets). Then:
...
The skill runs `slideless share <source_path> --title "Q4 Results" --json` and prints the returned `shareUrl`. Send the URL to anyone.
...
Runs `slideless update <shareId> ./q4-results`. The URL stays the same, recipients refresh and see the new content, and the view counter keeps incrementing from where it was. Unchanged assets (images, video, JS bundles) are deduplicated — only modified files re-upload.
...
| `blobs-missing` | Manifest references a hash not in the store | Should not happen in normal use — re-run `share-presentation` |
