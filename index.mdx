...
Same URL, new content, view counts preserved. Unchanged files (images, video, bundles) are deduplicated by content hash — only modified files re-upload.
...
| Upload from a script or CI pipeline | `slideless share ./my-deck --title "..." --strict --json` |
| Update a published deck without breaking the link | `slideless update <shareId> ./my-deck`. Same URL, view count preserved, unchanged files deduplicated |
| Freeze a recipient on a specific version | `slideless pin <shareId> <tokenId> --to-version N` |
| Track who's viewing | `slideless list` for totals, `slideless get <shareId>` for per-token counts |
| Send different links to different recipients | Multiple named tokens per presentation, each with its own `versionMode` |
| Manage everything from a UI | Dashboard at https://app.slideless.ai (browsing, preview, token management, archiving) |
...
- **Not a dashboard editor.** v3 uploads are CLI-driven. The dashboard handles browsing, previewing, token management, and archiving — not content editing.
